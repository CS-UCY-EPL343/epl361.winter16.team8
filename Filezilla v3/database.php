...
class Database {
	...
	public function putProduct($title, $message, $category, $price, $picture, $weight) {
		...
		echo $category;
		$FFF = "SELECT * FROM CATEGORIES WHERE CATEGORY_NAME = = \"". $gategory ."\"";
	...
	public function putCategory($PostTitle,$choose, $imgUrl) {
		
		echo "category function!";
		
		$FFF = "SELECT * FROM CATEGORIES ";
		$ffff1 = mysqli_query($conn, $FFF);
		$row = mysqli_fetch_array($ffff1, MYSQL_ASSOC);
		//echo $row['CATEGORY_ID'];
		//$picture = "pictures/boys(1).jpg";
		
		$sql = "INSERT INTO CATEGORIES (CATEGORY_ID,CATEGORY_NAME,SUPER_CATEGORY,DESCRIPTION)
		VALUES (
		'" . $PostTitle. "'
		'" . $choose. "'
		,'" . $imgUrl. "')";
