<?php
	...
	if(isset ($_POST['register-submit'])){
		...
		if($db -> register($email, $name, $surname, $password, $dob, $sex2, $address, $city, $postal_code, $mobile, $conn)){
			echo "<script> localStorage.setItem(\"islogin\",\"true\");localStorage.setItem(\"username\",\"".$db -> getUsername($email,$conn)."\");localStorage.setItem(\"email\",\"".$email."\") </script>";
		}
		...
		if($db -> login($email,$password,$conn)){
			echo "<script>localStorage.setItem(\"islogin\",\"true\"); localStorage.setItem(\"email\",\"".$email."\");localStorage.setItem(\"username\",\"".$db -> getUsername($email,$conn)."\") </script>";
